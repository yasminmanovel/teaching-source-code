...
while True:
    ...
    # Ball bouncing off bottom wall
    elif ball.ycor() < -290/2:
        ball.sety(-290/2)
        ball.dy *= -1
    ...
    if ball.xcor() > 400/2:
        ...
        pen.clear()
        pen.color("blue")
        pen.write("Player 1: {}  ".format(score_1), align="right", font=("Courier", 12, "normal"))
        pen.color("red")
        pen.write("  Player 2: {}".format(score_2), align="left", font=("Courier", 12, "normal"))
        ball.goto(0, 0)
        ball.dx *= -1

    # Ball hitting left wall
    elif ball.xcor() < -400/2:
        score_2 += 1
        pen.clear()
        pen.color("blue")
        pen.write("Player 1: {}  ".format(score_1), align="right", font=("Courier", 12, "normal"))
    ...
    # Ball hitting paddle
    if ball.xcor() < -340/2 and ball.ycor() < p1.ycor() + 50/2 and ball.ycor() > p1.ycor() - 50/2:
        ball.dx *= -1 
    
    elif ball.xcor() > 340/2 and ball.ycor() < p2.ycor() + 50/2 and ball.ycor() > p2.ycor() - 50/2:
        ball.dx *= -1
